...
for file in args.files:
    ...
    if extension == "csv":
        ...
        sys.stderr.write("File: " + file + " (via midicsv)" + "\n")
        try:
            rows = subprocess.check_output(["midicsv", file]).decode('utf-8').splitlines()
        except:
            print("Couldn't open '" + file + "'. Does it exist? Is midicsv installed?")
            continue
    elif extension == "mscx" or extension == "mscz":
        sys.stderr.write("File: " + file + " (via mscore and midicsv)" + "\n")
        try:
        ...
        sys.stderr.write("Ignoring: " + file + " (not a MIDI or CSV file)" + "\n")
        continue
    ...
    notes = [x for (y,x) in sorted(list(zip(onTicks,notes)), key=getKey)]
    onTicks.sort()
